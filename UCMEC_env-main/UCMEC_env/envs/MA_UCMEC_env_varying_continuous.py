...
N = 100  # number of APs
varsig = 4  # number of antennas of each AP
K = 4  # number of CPUs
...
bandwidth_a = 20e6  # bandwidth of access channel
...
Gm = 10  # Directivity gain of main lobes
Gs = 1 / 10  # Directivity gain of side lobes
Gain = np.array([Gm * Gm, Gm * Gs, Gs * Gs])  # random antenna gain in Eq. (7)
Gain_pro = np.array([(fai / (2 * math.pi)) ** 2, (2 * fai * (2 * math.pi - fai) / (2 * math.pi) ** 2) ** 2,
...
class MA_UCMEC(gym.Env):
    ...
    def step(self, action):
        ...
        # fronthaul delay
        front_delay_matrix = np.zeros([M, cluster_size])
