...
class MA_UCMEC(gym.Env):
    def __init__(self, render: bool = False):
        ...
        # a in {0,1,2,3,4}, p in {0, p_max} (totally 5 levels)
        ...
        self.action_space = spaces.Box(low=np.array(list(action_low) * M, dtype=np.float32),
                                       high=np.array(list(action_high) * M, dtype=np.float32), shape=((K + 1) * M_sim,),
                                       dtype=np.float32)
    ...
    def uplink_rate_cal(self, p):  # calculate the uplink transmit rate in Eq. (12)
        ...
        for i in range(M_sim):
            ...
            SINR_access_mole[i, 0] = (SINR_access_mole[i, 0] ** 2) * p[i, 0] * varsig
            for k in range(M_sim):
                if k == i:
                    ...
                    for j in range(N_sim):
                        if self.cluster_matrix[i, j] == 1:
                            SINR_access_inter[i, 0] = SINR_access_inter[i, 0] + theta[i, j] * beta[k, j] * p[k, 0]
    ...
    def step(self, action):
        ...
        if self.step_num == 1:  # The first step
            ...
            omega_current = np.zeros([M_sim, K])
            p_current = np.zeros([M_sim, 1])
            for i in range(M_sim):
                flag = K + 1
            ...
            print("Average Uplink Rate (Mbps):", np.sum(uplink_rate_access) / (M_sim*10e6))
        ...
        offload_task_size = np.zeros([M_sim, 1])
        for i in range(M_sim):
        ...
        for i in range(M_sim):
            for j in range(N_sim):
                for k in range(cluster_size):
                    if self.cluster_matrix[i, j] == 1:
                        front_delay_matrix[i, k] = offload_task_size[i, 0] / front_rate_user[i, j]
            front_delay[i, 0] = np.max(front_delay_matrix[i, :])


        # processing delay calculation
        # solve convex problem according to Eq. (24)
        proportion_list = np.zeros([M_sim, 1])  # offloading proportion of each user
        for i in range(M_sim):
