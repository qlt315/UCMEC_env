...
class MA_UCMEC_stat_coop(gym.Env):
    def __init__(self, render: bool = False):
        ...
        self.P_max = 0.05  # maximum transmit power of user / pilot power
        self.M_sim = 10  # number of users for simulation
        self.N_sim = 50  # number of APs for simulation
        self.reward_list = np.zeros([1,250])
    ...
    def step(self, action):
        ...
        if self.step_num % 200 == 0:
            self.reward_list[0,self.index] = np.sum(reward) / self.M_sim
            self.index = self.index + 1

        if self.step_num == 50000:
