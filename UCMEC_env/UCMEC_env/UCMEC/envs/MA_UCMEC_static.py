...
class MA_UCMEC_Static(gym.Env):
    def __init__(self, render: bool = False):
        ...
        # r in [0, 10e8]
        self.r_low = np.zeros(self.M_sim)
        self.r_high = np.ones(self.M_sim) * 1e8
        ...
        self.observation_space = spaces.Tuple(tuple([spaces.Box(low=self.r_low, high=self.r_high, shape=(self.n_agents,), dtype=np.float32)] * self.n_agents))
        # self.np_random = None
    ...
    def action_mapping(self,action_agent):
        # Transform the action space form MultiDiscrete to Discrete (1+4*5=21 cases)
        if action_agent == 0: # local processing
            omega_agent = 0
            p_agent = 0 
        elif action_agent == 1:
            omega_agent = 1
            ...
            omega_agent = 3
            p_agent = 0   
        elif action_agent == 10:
            omega_agent = 3
            p_agent = 1   
        elif action_agent == 11:
            omega_agent = 3
            p_agent = 2  
        elif action_agent == 12:
            omega_agent = 3
            p_agent = 3
            ...
            for j in range(self.K):
                if chi[i, j] == 1:
                    if self.link_type[i, j] == 0:  # LOS link
                        ...
                        SINR_front_mole = self.p_ap * self.G[i, j] * pow(self.distance_matrix_front[i, j] / 1000, self.alpha_nlos)
    ...
    def reset(self):
        reset_state = np.random.uniform(low=0, high=1e6, size=(self.M_sim,))
    ...
    def step(self, action):
        ...
        for i in range(self.M_sim):
            reward[i, 0] = - np.sum(np.maximum(local_delay, front_delay + uplink_delay + actual_process_delay)) / self.M_sim
        ...
        info = {}
        print("Reward:",np.sum(reward)/self.M_sim)
        return tuple(observation), list(reward), done, info
