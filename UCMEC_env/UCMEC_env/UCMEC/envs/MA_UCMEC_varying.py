...
class MA_UCMEC_Varying(gym.Env):
    def __init__(self, render: bool = False):
        ...
        self.observation_space = spaces.Tuple(
            tuple([spaces.Box(low=-np.inf, high=+np.inf, shape=(self.n_agents,), dtype=np.float32)] * self.n_agents))
        # self.np_random = None
    ...
    def step(self, action):
        ...
        for i in range(self.M_sim):
            reward[i, 0] = - np.sum(
                np.maximum(local_delay, front_delay + uplink_delay + actual_process_delay)) / self.M_sim
